...
HttpListener BeginCallbackListener(string callbackUrl)
{
...
async Task<Dictionary<string, string>> AwaitOAuthCallbackAsync(HttpListener listener)
{
  try
  {
    while (true)
...
Process? TryStartOAuthCaptureProcess(string callbackUrl, string state)
{
    try
    {
        var psi = new ProcessStartInfo
...
bool IsHttpLoopbackCallback(string callbackUrl)
{
...
async Task<Dictionary<string, string>> AwaitOAuthCaptureProcessAsync(Process process)
{
...
void PresentAuthorizeUrl(string profileName, string authorizeUrl, string browserLaunch)
{
