...
public sealed class CsFileEmitter : IScriptEmitter
{
    ...
    {
        sb.AppendLine("static async Task<(string AccessToken, string? RefreshToken, int ExpiresIn, string? IdToken)?> PostTokenRequestAsync(HttpClient http, JsonElement profile, string tokenUrl, string clientId, Dictionary<string, string> form, JsonElement? secrets)");
        sb.AppendLine("{");
        ...
        sb.AppendLine("static Task<(string AccessToken, string? RefreshToken, int ExpiresIn, string? IdToken)?> ExchangeCodeForTokenAsync(HttpClient http, JsonElement profile, string tokenUrl, string clientId, string code, string verifier, string callbackUrl, JsonElement? secrets) =>");
        ...
        sb.AppendLine("static Task<(string AccessToken, string? RefreshToken, int ExpiresIn, string? IdToken)?> FetchClientCredentialsTokenAsync(HttpClient http, JsonElement profile, string tokenUrl, string clientId, JsonElement? secrets) =>");
        ...
        sb.AppendLine("static Task<(string AccessToken, string? RefreshToken, int ExpiresIn, string? IdToken)?> RefreshOAuthTokenAsync(HttpClient http, JsonElement profile, string tokenUrl, string clientId, string refreshToken, JsonElement? secrets) =>");
    ...
    private static void AppendOAuthTokenCacheFunctions(StringBuilder sb)
    {
        ...
        sb.AppendLine("static void StoreTokenInCache(Dictionary<string, JsonElement> cache, string profileName, (string AccessToken, string? RefreshToken, int ExpiresIn, string? IdToken) token)");
        sb.AppendLine("{");
    ...
    {
        sb.AppendLine("static Process? TryStartOAuthCaptureProcess(string callbackUrl, string state)");
        sb.AppendLine("{");
        sb.AppendLine("    try");
        sb.AppendLine("    {");
        sb.AppendLine("        var psi = new ProcessStartInfo");
        ...
        sb.AppendLine("        var process = Process.Start(psi);");
        sb.AppendLine("        if (process is null) return null;");
        sb.AppendLine("        // Old/wrong binaries exit immediately — treat as missing so HTTP can fall back.");
        sb.AppendLine("        process.WaitForExit(150);");
        sb.AppendLine("        if (process.HasExited)");
        sb.AppendLine("        {");
        sb.AppendLine("            process.Dispose();");
        sb.AppendLine("            return null;");
        sb.AppendLine("        }");
        ...
        sb.AppendLine("static bool IsHttpLoopbackCallback(string callbackUrl)");
        sb.AppendLine("{");
        sb.AppendLine("    if (!Uri.TryCreate(callbackUrl, UriKind.Absolute, out var uri)) return false;");
        sb.AppendLine("    if (!uri.Scheme.Equals(\"http\", StringComparison.OrdinalIgnoreCase)) return false;");
        sb.AppendLine("    return uri.Host.Equals(\"localhost\", StringComparison.OrdinalIgnoreCase)");
        sb.AppendLine("        || uri.Host is \"127.0.0.1\" or \"::1\" or \"[::1]\";");
        sb.AppendLine("}");
        sb.AppendLine();

        sb.AppendLine("static async Task<Dictionary<string, string>> AwaitOAuthCaptureProcessAsync(Process process)");
        sb.AppendLine("{");
        sb.AppendLine("    var stdout = await process.StandardOutput.ReadToEndAsync();");
        sb.AppendLine("    await process.WaitForExitAsync();");
        sb.AppendLine("    var line = stdout.Trim();");
        sb.AppendLine("    if (line.Length == 0)");
        sb.AppendLine("        throw new AuthResolutionException($\"api2skill oauth-capture exited {process.ExitCode} with empty stdout.\");");
        sb.AppendLine("    try");
        sb.AppendLine("    {");
        sb.AppendLine("        using var doc = JsonDocument.Parse(line);");
        sb.AppendLine("        var root = doc.RootElement;");
        sb.AppendLine("        var result = new Dictionary<string, string>(StringComparer.Ordinal);");
        sb.AppendLine("        if (root.TryGetProperty(\"code\", out var codeEl) && codeEl.GetString() is { } code) result[\"code\"] = code;");
        sb.AppendLine("        if (root.TryGetProperty(\"state\", out var stateEl) && stateEl.GetString() is { } st) result[\"state\"] = st;");
        sb.AppendLine("        if (root.TryGetProperty(\"error\", out var errEl) && errEl.GetString() is { } err) result[\"error\"] = err;");
        sb.AppendLine("        if (root.TryGetProperty(\"errorDescription\", out var edEl) && edEl.GetString() is { } ed) result[\"error_description\"] = ed;");
        sb.AppendLine("        var ok = root.TryGetProperty(\"ok\", out var okEl) && okEl.ValueKind == JsonValueKind.True;");
        sb.AppendLine("        if (!ok && !result.ContainsKey(\"code\"))");
        sb.AppendLine("            throw new AuthResolutionException(result.TryGetValue(\"error\", out var e) ? e : \"oauth-capture failed\");");
        sb.AppendLine("        return result;");
        sb.AppendLine("    }");
        sb.AppendLine("    catch (JsonException ex)");
        sb.AppendLine("    {");
        sb.AppendLine("        throw new AuthResolutionException($\"Failed to parse oauth-capture JSON: {ex.Message}\");");
        sb.AppendLine("    }");
        sb.AppendLine("}");
        sb.AppendLine();

        sb.AppendLine("static void PresentAuthorizeUrl(string profileName, string authorizeUrl, string browserLaunch)");
        sb.AppendLine("{");
        ...
        sb.AppendLine("    var captureProc = TryStartOAuthCaptureProcess(callbackUrl, state);");
        sb.AppendLine("    Dictionary<string, string> callbackParams;");
        sb.AppendLine("    if (captureProc is not null)");
        sb.AppendLine("    {");
        sb.AppendLine("        Console.WriteLine($\"Listening for OAuth callback on {callbackUrl} ...\");");
        sb.AppendLine("        await Task.Delay(200);");
        sb.AppendLine("        PresentAuthorizeUrl(profileName, authorizeUrl, GetProp(profile.Value, \"browserLaunch\"));");
        sb.AppendLine("        try { callbackParams = await AwaitOAuthCaptureProcessAsync(captureProc); }");
        sb.AppendLine("        catch (AuthResolutionException ex) { Console.Error.WriteLine(ex.Message); return 2; }");
        sb.AppendLine("        finally { captureProc.Dispose(); }");
        sb.AppendLine("    }");
        sb.AppendLine("    else if (IsHttpLoopbackCallback(callbackUrl))");
        sb.AppendLine("    {");
        ...
        sb.AppendLine("    else");
        sb.AppendLine("    {");
        sb.AppendLine("        Console.Error.WriteLine(\"Could not start 'api2skill oauth-capture'. Install or upgrade api2skill and ensure it is on PATH. HTTPS / custom-scheme / hosted callbacks require the tool (HTTP loopback can fall back to an in-script listener).\");");
        sb.AppendLine("        return 2;");
        sb.AppendLine("    }");
        sb.AppendLine();
